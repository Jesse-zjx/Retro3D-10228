"""
original version:https://github.com/pytorch/fairseq/blob/main/scripts/average_checkpoints.py
"""
import argparse
import collections
import os
...
def average_checkpoints(inputs):
    ...
    for k, v in params_dict.items():
        ...
        if averaged_params[k].is_floating_point():
            ...
            averaged_params[k].div_(num_models, rounding_mode='floor')
...
def last_n_checkpoints(paths, n, update_based, upper_bound=None):
    ...
    path = paths[0]
    if update_based:
        pt_regexp = re.compile(r"epoch:\d+_(\d+)_model\.chkpt")
    else:
        pt_regexp = re.compile(r"epoch:(\d+)_model\.chkpt")
